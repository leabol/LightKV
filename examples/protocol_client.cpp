...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    uint32_t total = (static_cast<uint32_t>(hdr[0]) << 24) | (static_cast<uint32_t>(hdr[1]) << 16) |
    ...
    if (val_len) {
      val.resize(val_len);
      if (!read_n(fd, reinterpret_cast<uint8_t*>(val.data()), val_len)) {
        cerr << "recv failed\n";
        break;
      }
    }
