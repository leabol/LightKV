...
void warmup_phase(const Config& cfg, std::atomic<uint64_t>& ok_count, std::atomic<uint64_t>& fail_count) {
    try {
        net::Socket sock;
...
void get_worker(const Config& cfg,
    ...
    try {
        ...
        std::uniform_int_distribution<uint64_t> key_dist(0, cfg.warmup_keys == 0 ? 0 : cfg.warmup_keys - 1);
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "load test config:"
                  << " host=" << cfg.host
                  << " port=" << cfg.port
                  << " warmup_keys=" << cfg.warmup_keys
                  << " threads=" << cfg.threads
                  << " requests/thread=" << cfg.requests_per_thread
        ...
        if (!failures.empty()) {
            ...
            for (const auto& sample : failures) {
                std::cout << "  key=" << sample.key
                          << " status=0x" << std::hex << std::uppercase << static_cast<int>(sample.status)
                          << std::dec << " message=" << sample.message << '\n';
            }
        }
